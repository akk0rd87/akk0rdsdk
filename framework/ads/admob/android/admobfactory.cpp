...
std::shared_ptr<ads::AdMob::Provider> ads::AdMob::createProvider(std::weak_ptr<ads::ProviderCallback> callback, ads::Format format) {
    auto provider = std::make_shared<ads::AdMob::AndroidProvider>(static_cast<JNIEnv*>(SDL_AndroidGetJNIEnv()), static_cast<jobject>(SDL_AndroidGetActivity()), callback, format);
